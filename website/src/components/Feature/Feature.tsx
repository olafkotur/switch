...
export default class Feature extends React.Component<IProps> {
  render() {
    return (
      <ButtonBase className="feature-spacing" onClick={() => this.props.handleClick(this.props.id)}>
        <Paper
          elevation={0}
          className={`d-flex justify-content-center align-items-center bg-secondary feature-container ${
            this.props.show ? 'feature-container-expanded' : ''
          }`}
        >
